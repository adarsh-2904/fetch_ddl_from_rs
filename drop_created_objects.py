...
def load_config(config_file="create_objects_parameters.yaml"):
    try:
        ...
        yaml_path = base_path / "Create_Object_Input" / config_file
...
# Find the latest created CSV file (now with object_type specific directory)
def find_latest_created_csv(output_base_directory, object_type, run_identifier):
    try:
        # Construct path to object-type specific directory
        object_output_dir = os.path.join(output_base_directory, object_type.capitalize())

        # Pattern to match: run_identifier_TIMESTAMP_created.csv
        pattern = os.path.join(object_output_dir, f"{run_identifier}_*_created.csv")
        ...
        print(f"  Directory: {object_output_dir}")
...
# Initialize drop summary CSV (now in object-type specific directory)
...
def main():
    ...
    if not created_file:
        print(f"\n❌ No created CSV file found for {object_type.upper()}. Nothing to drop.")
        return
    ...
    try:
        ...
        # Prepare summary file path with timestamp in object-type specific directory
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        object_output_dir = os.path.join(output_base_directory, object_type.capitalize())
        summary_file = os.path.join(object_output_dir, f"{run_identifier}_{timestamp}_dropped_summary.csv")
