CREATE OR REPLACE VIEW mktg_ops_vws.bzfc_prospect_pt_scr_nonfin AS 
-- mktg_ops_vws.bzfc_prospect_pt_scr_nonfin source
...
from Target Analytics for all ARC Non-financial prospects.  
...
------------------------------------------------------------------------------------------------------------------------------------ */
mktg_ops_vws.bzfc_prospect_pt_scr_nonfin
AS 
SELECT 
cnst_mstr_id, 
past_gvr_typ_rtng ,
CASE
   ...
   END AS past_gvr_typ_dsc , -- (TITLE 'Past Giver Type Description'),
vlcty_rtng ,
CASE
...
--annuity_like (TITLE 'Annual Gift Likelihood (0-1000)') as ann_like , 
crt_like  as pg_like, -- alias for crtlike (Charitable Remainder Trust Likelihood)
...
WHERE cnst_typ_cd = 'N' -- Non-financial prospects only
   -- Filter any constituents also appearing as Financial prospects
   AND cnst_mstr_id NOT IN (SELECT DISTINCT cnst_mstr_id FROM mktg_ops_vws.bzfc_prospect_pt_scr_fin)
