CREATE OR REPLACE VIEW mktg_ops_vws.srvy_vlntr_los AS 
-- mktg_ops_vws.srvy_vlntr_los source

create or REPLACE VIEW mktg_ops_vws.srvy_vlntr_los
/*
Created By: 	Michael Andrien
Create Date:	05/29/2024
Purpose: This view was create to support the FY25 volunteer survey requirement to pull the volunteer line of service (LOS) into the survey from the EDW
rather than asking the volunteer to select their active service areas within the survey.  The view contains one row per volunteer and included the CDI master id,
and an indicator attribute for each service area name defined within the Volunteer Connection (VCN)
positions taxonomy table.  The view also contains a concatenated LOS column that is referenced within the Adobe Campaign
schema and used by the MODS Ops team to determine which LOS questions to include in the triggered volunteer survey.  The MODS Ops team will use text search logic
to scan the concatenated attribute for the LOS details.  The current LOS names are listed below.  
	Biomedical Services
	...
	Legal (OGC)/Risk Management
	Marketing
	Non-Core Community Services
	Operations
	Real Estate Services
	Services for Military and Veteran Communities
	Sustainability
	Training Services
	Volunteer Services
...
Update Date:	06/04/2024
Purpose:  Added the following indicator attributes
	CASE WHEN non_core_cs_cnt > 0 THEN 1 ELSE 0 END AS non_core_cs_ind,
	CASE WHEN ops_cnt > 0 THEN 1 ELSE 0 END AS ops_ind,
	CASE WHEN re_srvc_cnt > THEN 1 ELSE 0 END AS re_srvc_ind,
...
Modified Date:	08/15/2024
Purpose: Updated the Volunteer Services description from Training Services (CPR, First Aid, etc.) to Volunteer Services

Modified By:	Michael Andrien
Modified Date:	01/02/2025
Purpose: Added the join the the cnst_mstr_bridge view and changed the join column on the dim_volunteer join to ensure we pickup all volunteer profiles associated with the CDI Master.

Modified By:	Michael Andrien
Modified Date:	01/31/2025
Purpose:  Update the saf_srvc_cnt variable to include all 3 service names related to SAF.  The first two in the list below were added.
...
Purpose:  Commented out the test cnst_mstr_id 
*/
AS
SELECT
	a.cnst_mstr_id, 
...
--AND vp.cnst_mstr_id = 164300400
GROUP BY 1
) a (cnst_mstr_id, biomed_srvc_cnt, comm_pa_srvc_cnt, corp_fin_srvc_cnt, corp_mgt_srvc_cnt, dev_fr_srvc_cnt, distr_srvc_cnt, divrsty_srvc_cnt,gov_rel_srvc_cnt,hr_srvc_cnt,
