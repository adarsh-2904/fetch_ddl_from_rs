CREATE OR REPLACE PROCEDURE mktg_ops_tbls.sp_adb_arcreportcellcode()
 LANGUAGE plpgsql
AS $$
...
BEGIN
    ...
    WHERE process_nm = 'sp_adb_arcreportcellcode'
      AND LOWER(table_nm) = 'adb_arcreportcellcode';
    ...
    VALUES ('sp_adb_arcreportcellcode', 'Stored Procedure', 'Inprogress', v_start_time);	

	--step 3: Prep temp data to bulk insert
--drop table if exists existingrecord;
	create temp table existingrecord as
    SELECT 
        stg.iReportCellCodeId,
        stg.sHighDate,
        stg.sHighDonation,
        stg.sLowDate,
        stg.sLowDonation,
        stg.sTag,
		case when tgt.iReportCellCodeId is null then 'I' else 'C' end as row_stat_cd,
        v_appl_src_cd as appl_src_cd,
        v_dw_load_id as load_id,
        CURRENT_TIMESTAMP
    FROM mods_bi_rep.mktg_stage_tbls.stg_adb_arcreportcellcode stg
		 LEFT OUTER JOIN mktg_ops_tbls.adb_arcreportcellcode tgt
		ON stg.iReportCellCodeId = tgt.iReportCellCodeId;	
  
  ---Step 3: delete from tgt where src matches the conditions.
	DELETE FROM mktg_ops_tbls.adb_arcreportcellcode
	USING mods_bi_rep.mktg_stage_tbls.stg_adb_arcreportcellcode stg
	WHERE stg.iReportCellCodeId = mktg_ops_tbls.adb_arcreportcellcode.iReportCellCodeId;	
	
	
	
	-- Step 4: insert all stg records into tgt-ops table.
    INSERT INTO mktg_ops_tbls.adb_arcreportcellcode (
        iReportCellCodeId,
        sHighDate,
        sHighDonation,
        sLowDate,
    ...
    FROM mods_bi_rep.mktg_stage_tbls.stg_adb_arcreportcellcode;

    drop table existingrecord;	
    CALL etl_config.sp_upsert_load_control(
        'sp_adb_arcreportcellcode', 'adb_arcreportcellcode',--this will be updated in every stored proc
	...
	-- Step 6: Audit update	
       ...
       WHERE 
           proc_name = 'sp_adb_arcreportcellcode' 
    ...
    VALUES ('sp_adb_arcreportcellcode', 'Stored Procedure', 'ERROR', v_start_time,v_end_time,SUBSTRING(SQLERRM, 1, 500));
