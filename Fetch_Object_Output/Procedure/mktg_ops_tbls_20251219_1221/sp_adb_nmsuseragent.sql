CREATE OR REPLACE PROCEDURE mktg_ops_tbls.sp_adb_nmsuseragent()
 LANGUAGE plpgsql
AS $$
...
BEGIN
    ...
    WHERE process_nm = 'sp_adb_nmsuseragent'
      AND LOWER(table_nm) = 'adb_nmsuseragent';
    ...
    VALUES ('sp_adb_nmsuseragent', 'Stored Procedure', 'Inprogress', v_start_time);	
...
--drop table if exists existingrecord;
	create temp table existingrecord as
        ...
        stg.sosimage, 
        stg.sosname,
		case when tgt.ihashkey is null then 'I' else 'C' end as row_stat_cd,
		v_appl_src_cd as appl_src_cd,
		v_dw_load_id as load_id,
        CURRENT_DATE -- Insert the current date into the dw_trans_ts column
    FROM
       mods_bi_rep.mktg_stage_tbls.stg_adb_nmsuseragent stg
		 LEFT OUTER JOIN mktg_ops_tbls.adb_nmsuseragent tgt
		ON stg.ihashkey = tgt.ihashkey;
		
    ---Step 4: delete from tgt where src matches the conditions.
	DELETE FROM mktg_ops_tbls.adb_nmsuseragent
	USING mods_bi_rep.mktg_stage_tbls.stg_adb_nmsuseragent stg
	WHERE stg.ihashkey = mktg_ops_tbls.adb_nmsuseragent.ihashkey;	
	
	
	
	-- Step 5: insert all stg records into tgt-ops table.
    INSERT INTO mktg_ops_tbls.adb_nmsuseragent (
        ihashkey, 
        sbrowserimage, 
        sbrowsername, 
        sbrowserversion, 
		...
		appl_src_cd	,
		load_id,
        dw_trans_ts -- Add the column for the transaction timestamp
    )
    SELECT 
        stg.ihashkey, 
        stg.sbrowserimage, 
  ...
  -- Capture number of rows inserted
    ...
    FROM mods_bi_rep.mktg_stage_tbls.stg_adb_nmsuseragent;

    	
    CALL etl_config.sp_upsert_load_control(
        'sp_adb_nmsuseragent', 'adb_nmsuseragent',--this will be updated in every stored proc
    ...
    VALUES ('sp_adb_nmsuseragent', 'Stored Procedure', 'ERROR', v_start_time,v_end_time,SUBSTRING(SQLERRM, 1, 500));
