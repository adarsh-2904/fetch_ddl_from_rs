CREATE OR REPLACE PROCEDURE mktg_ops_tbls.sp_adb_nmsinsms()
 LANGUAGE plpgsql
AS $$
...
BEGIN
    ...
    WHERE process_nm = 'sp_adb_nmsinsms'
      AND LOWER(table_nm) = 'adb_nmsinsms';
    ...
    VALUES ('sp_adb_nmsinsms', 'Stored Procedure', 'Inprogress', v_start_time);	
	...
	SELECT
        ...
        stg.tsReceival,
        stg.ibroadLogRCPId,
        case when tgt.iinsmsid is null then 'I' else 'C' end as row_stat_cd,
        v_appl_src_cd as appl_src_cd,
        v_dw_load_id as load_id,
        CURRENT_DATE as dw_trans_ts
    FROM
        mods_bi_rep.mktg_stage_tbls.stg_adb_NmsInSMS stg
		 LEFT OUTER JOIN mktg_ops_tbls.adb_nmsinsms tgt
		ON stg.iinsmsid = tgt.iinsmsid;
		
    ---Step 3: delete from tgt where src matches the conditions.
	DELETE FROM mktg_ops_tbls.adb_nmsinsms
	USING mods_bi_rep.mktg_stage_tbls.stg_adb_nmsinsms stg
	WHERE stg.iinsmsid = mktg_ops_tbls.adb_nmsinsms.iinsmsid;	
	
	
	
	-- Step 4: insert all stg records into tgt-ops table.
    INSERT INTO mktg_ops_tbls.adb_nmsinsms (
        icountrycode,
        iextaccountid,
        iinsmsid,
        ioperatorcode,
...
-- Capture number of rows inserted
    ...
    SELECT COALESCE(MAX(dw_trans_ts), '9999-01-01'::timestamp)--modified to dw_trans_ts from tslastmodified
    INTO v_next_extract_ts
    FROM mods_bi_rep.mktg_stage_tbls.stg_adb_nmsinsms;

    	
    CALL etl_config.sp_upsert_load_control(
        'sp_adb_nmsinsms', 'adb_nmsinsms',--this will be updated in every stored proc
    ...
    VALUES ('sp_adb_nmsinsms', 'Stored Procedure', 'ERROR', v_start_time,v_end_time,SUBSTRING(SQLERRM, 1, 500));
