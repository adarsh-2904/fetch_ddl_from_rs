CREATE OR REPLACE PROCEDURE mktg_ops_tbls.sp_adb_nmsexcludelogrcp()
 LANGUAGE plpgsql
AS $$
/*
...
BEGIN
    ...
    WHERE process_nm = 'sp_adb_nmsexcludelogrcp'
      AND LOWER(table_nm) = 'adb_nmsexcludelogrcp';
    ...
    VALUES ('sp_adb_nmsexcludelogrcp', 'Stored Procedure', 'Inprogress', v_start_time);	
...
--drop table if exists existingrecord;
	create temp table existingrecord as
		...
		stg.istatus, stg.saddress, stg.tsevent, stg.tslastmodified,
	...
	FROM mods_bi_rep.mktg_stage_tbls.stg_adb_nmsexcludelogrcp stg
		 LEFT OUTER JOIN mktg_ops_tbls.adb_nmsexcludelogrcp tgt
		ON stg.ibroadlogid = tgt.ibroadlogid;
		
    ---Step 4: delete from tgt where src matches the conditions.
	DELETE FROM mktg_ops_tbls.adb_nmsexcludelogrcp
	USING mods_bi_rep.mktg_stage_tbls.stg_adb_nmsexcludelogrcp stg
	WHERE stg.ibroadlogid = mktg_ops_tbls.adb_nmsexcludelogrcp.ibroadlogid;	
	
	
	
	-- Step 5: insert all stg records into tgt-ops table.
    INSERT INTO mktg_ops_tbls.adb_nmsexcludelogrcp (
        ibroadlogid, ideliveryid, iflags, imsgid, irecipientid, istatus, saddress,
        tsevent, tslastmodified, row_stat_cd, appl_src_cd, load_id, dw_trans_ts
    )
	    SELECT  ibroadlogid, ideliveryid, iflags, imsgid, irecipientid, istatus, saddress,
        tsevent, tslastmodified, row_stat_cd, appl_src_cd, load_id, dw_trans_ts from existingrecord;
    ...
    FROM mods_bi_rep.mktg_stage_tbls.stg_adb_nmsexcludelogrcp;

    	
    CALL etl_config.sp_upsert_load_control(
        'sp_adb_nmsexcludelogrcp', 'adb_nmsexcludelogrcp',--this will be updated in every stored proc
	...
	-- Step 7: Audit update	
       ...
       WHERE 
           proc_name = 'sp_adb_nmsexcludelogrcp' 
    ...
    VALUES ('sp_adb_nmsexcludelogrcp', 'Stored Procedure', 'ERROR', v_start_time,v_end_time,SUBSTRING(SQLERRM, 1, 500));
