CREATE OR REPLACE PROCEDURE mktg_ops_tbls.sp_adb_nmssubscription()
 LANGUAGE plpgsql
AS $$
/*
...
BEGIN
	...
	-- Step 1: Fetch parameters from control table
	    ...
	    WHERE process_nm = 'sp_adb_nmssubscription'
	      AND LOWER(table_nm) = 'adb_nmssubscription';
	...
	-- Step 2b: insert audit log InProgress status 
	    ...
	    VALUES ('sp_adb_nmssubscription', 'Stored Procedure', 'Inprogress', v_start_time);
	...
	--step 3: select existing record
		...
		SELECT 
	        ...
	        stg.iRecipientId,
	        stg.iServiceId,
	        stg.sAddressSpecific,
	        stg.tsCreated,
	        stg.tsExpiration,
	        case when tgt.iRecipientId is null then 'I' else 'C' end as row_stat_cd,
	        v_appl_src_cd as appl_src_cd,
	        v_dw_load_id as load_id,
	        CURRENT_DATE as dw_trans_ts
	    FROM mods_bi_rep.mktg_stage_tbls.stg_adb_nmssubscription stg
	    LEFT JOIN mktg_ops_tbls.adb_nmssubscription tgt 
	        ON stg.iRecipientId = tgt.iRecipientId 
	        AND stg.iServiceId = tgt.iServiceId;

	---Step 4: delete from tgt where src matches the conditions.

		delete from  mktg_ops_tbls.adb_nmssubscription
   ...
   -- Step 5: insert all stg records into tgt-ops table.
	    INSERT INTO mktg_ops_tbls.adb_nmssubscription (
	        iConfirmationId,
	        iDeleteStatus,
	        iEmailFormat,
	        iRecipientId,
    ...
    -- Step 6: Get latest timestamp for next run and Update load control	
		...
		CALL etl_config.sp_upsert_load_control(
	        'sp_adb_nmssubscription', 'adb_nmssubscription',
	...
	-- Step 7: Audit update	
       ...
       WHERE 
           proc_name = 'sp_adb_nmssubscription' 
	...
	-- Step 8: Insert in audit to Error
	    ...
	    VALUES ('sp_adb_nmssubscription', 'Stored Procedure', 'ERROR', v_start_time,v_end_time,SUBSTRING(SQLERRM, 1, 500));
END;
