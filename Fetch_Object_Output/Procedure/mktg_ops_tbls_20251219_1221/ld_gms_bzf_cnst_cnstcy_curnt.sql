CREATE OR REPLACE PROCEDURE mktg_ops_tbls.ld_gms_bzf_cnst_cnstcy_curnt()
 LANGUAGE plpgsql
AS $_$
/*
Created by: Michael Andrien
Created date: 02/29/2020
Purpose: This macro flattened 'frf_cnst_cnstcy' data and loads into ufds_tbls.bzf_cnst_cnstcy table . Full Data Load.  NOTE: This is a copy of the UFDS_TBLS verion, but we've added the active_ind = 1 to limit the view to active constituencies

Modified by: Michael Andrien
Modified date: 01/08/2021
Purpose: Removed the Table Lock statements from teh macro since the query references views.  The locks were causing grant issues when executing the macro.

Modified by: Michael Andrien
Modified date: 02/18/2021
Purpose: Modified the case statement for setting the tifny_crcle_bmh_mmbr_ind attribute to  include both cntstcy_typ values below.  Prior to this update only the 'Tiffany Circle BMH Member' description was included in the definition.
...
Purpose: Added the Leadership Society Gold attribute
...
BEGIN
    ...
    VALUES ('ld_gms_bzf_cnst_cnstcy_curnt', 'Stored Procedure', 'Inprogress', v_start_time);
...
truncate table mktg_stage_tbls.gms_bzf_cnst_cnstcy_curnt_stg;

/*  Inserting all records into the ufds_tbls.bzf_cnst_cnstcy */

insert into mktg_stage_tbls.gms_bzf_cnst_cnstcy_curnt_stg
with cte as (
Select 
...
(
	...
	WHERE 
		frf_cnst_cnstcy.active_ind = 1 and 
...
truncate table mktg_ops_tbls.gms_bzf_cnst_cnstcy_curnt;

insert into mktg_ops_tbls.gms_bzf_cnst_cnstcy_curnt select * from  mktg_stage_tbls.gms_bzf_cnst_cnstcy_curnt_stg; 


	
	--audit update	
	v_end_time := GETDATE();
	v_ok_message = cast((select count(*) from mktg_ops_tbls.gms_bzf_cnst_cnstcy_curnt) as nvarchar)+ ' Records inserted.';
        UPDATE mods_bi.etl_config.audit_log
        SET status = 'Complete', end_time = v_end_time,TaskMessage = v_ok_message
        WHERE proc_name = 'ld_gms_bzf_cnst_cnstcy_curnt' AND task_name = 'Stored Procedure' AND start_time = v_start_time;
...
--Insert in audit to Error
    ...
    VALUES ('ld_gms_bzf_cnst_cnstcy_curnt', 'Stored Procedure', 'ERROR', v_start_time,v_end_time,SUBSTRING(SQLERRM, 1, 500));
