CREATE OR REPLACE PROCEDURE mktg_ops_tbls.ld_srvy_anvrsy_vlntr_rspns_all()
 LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    VALUES ('ld_srvy_anvrsy_vlntr_rspns_all', 'Stored Procedure', 'Inprogress', v_start_time);
	
	BEGIN
		/* First, truncate data from the table then insert the new data */
		Truncate table mktg_stage_tbls.srvy_anvrsy_vlntr_rspns_all_stg;

		--1st Insert
		/* Now reload the table with the current data */
		/* FY20 Survey v3.0 (vmsAnnSurvey3) */
		INSERT INTO mktg_stage_tbls.srvy_anvrsy_vlntr_rspns_all_stg
		WITH ranked_data AS (
			SELECT
				snapshot_ts, 
				history_record_ts, 
				...
				a6_vlntr_sprt_rtng,
				a6_vlntr_sprt_rtng_dsc,
				b6_expctn_rtng,
				b6_expctn_rtng_dsc,
				c6_trng_mtrl_rtng,
				c6_trng_mtrl_rtng_dsc,
				tmwrk_good_rtng,
				tmwrk_good_rtng_dsc,
				d6_tmwrk_good_rtng,
				d6_tmwrk_good_rtng_dsc,
				e6_team_value_rtng,
				e6_team_value_rtng_dsc,
				f6_rspct_sprvsr_rtng,
				f6_rspct_sprvsr_rtng_dsc,
				g6_bhvr_cnstnt_rtng,
				g6_bhvr_cnstnt_rtng_dsc,
				h6_prspctv_value_rtng,
				h6_prspctv_value_rtng_dsc,
				i6_vlntr_cont_rtng,
				i6_vlntr_cont_rtng_dsc,
				j6_bkgrnd_rspctd_rtng,
				j6_bkgrnd_rspctd_rtng_dsc,
				vlntr_hrs_value,
				vlntr_hrs_dsc,
				a9_dntd_bld_ind,
				b9_dntd_mny_ind,
				...
				load_id,
				dm_cnst_zip_5_cd,
				COALESCE(attribtn_zip_5_cd, '00000') AS attribtn_zip_5_cd,
				unit_key,
				mktg_unit_key,
				mktg_unit_cd,
				unit_nm,
			...
			FROM (
				...
				LEFT JOIN mktg_ops_tbls.dim_unit_military_station_zips msz ON svp.unit_cd = msz.nk_ecode
				LEFT JOIN mktg_ops_tbls.dim_unit_military_station_zips msz_mktg ON svp.mktg_unit_cd = msz_mktg.nk_ecode
				LEFT JOIN mktg_ops_vws.dim_unit_merged um1 ON CASE
						WHEN COALESCE(msz.nk_ecode, '0') <> '0' THEN msz.nk_ecode
						WHEN COALESCE(msz_mktg.nk_ecode, '0') <> '0' THEN msz_mktg.nk_ecode
						ELSE '00000'
					END = um1.nk_ecode
				LEFT JOIN mktg_ops_vws.dim_unit_merged um2 ON CASE
				...
				LEFT JOIN eda.arc_goats_subscr_vws.bzfc_taxo_humanitarian_curr gs ON gs.zipcode_cd = CASE 
						WHEN um1.nk_ecode <> '00000' AND um1.unit_zip IS NOT NULL THEN COALESCE(um1.unit_zip, '00000')
						WHEN um2.nk_ecode <> '00000' AND um2.unit_zip IS NOT NULL THEN COALESCE(um2.unit_zip, '00000')
						ELSE svp.dm_cnst_zip_5_cd
					END
				LEFT JOIN eda.dw_common_vws.dim_zipcodes dz ON dz.zip = CASE 
						WHEN um1.unit_zip IS NOT NULL THEN um1.unit_zip 
						ELSE um2.unit_zip 
					END
		...
		SELECT
			snapshot_ts, history_record_ts, history_record_id, adnc_mbr_id, cnst_mstr_id, orig_cnst_mstr_id, last_nm, email, 
			survey_nm, vlntr_length, a5_acctg_fnc_ind, a2_blood_svc_ind, b2_gen_admin_sprt_srvc_ind, c2_saf_srvc_ind, 
			d2_comnty_srvc_ind, e2_health_safety_srvc_ind, f2_vlntr_srvc_ind, g2_dstr_srvc_ind, h2_intl_srvc_ind, 
			i2_youth_prgrm_srvc_ind, j2_fr_srvc_ind, k2_ldrshp_ind, m5_mrktg_ind, l2_other_srvc_ind, vol_area_no_answer, 
			vlntr_xprnc_rtng, vlntr_xprnc_rtng_dsc, rcmnd_frnd_vlntr_rtng, rcmnd_frnd_vlntr_rtng_dsc, a6_vlntr_sprt_rtng, 
			a6_vlntr_sprt_rtng_dsc, b6_expctn_rtng, b6_expctn_rtng_dsc, c6_trng_mtrl_rtng, c6_trng_mtrl_rtng_dsc, 
			tmwrk_good_rtng, tmwrk_good_rtng_dsc, d6_tmwrk_good_rtng, d6_tmwrk_good_rtng_dsc, e6_team_value_rtng, 
			e6_team_value_rtng_dsc, f6_rspct_sprvsr_rtng, f6_rspct_sprvsr_rtng_dsc, g6_bhvr_cnstnt_rtng, g6_bhvr_cnstnt_rtng_dsc, 
			h6_prspctv_value_rtng, h6_prspctv_value_rtng_dsc, i6_vlntr_cont_rtng, i6_vlntr_cont_rtng_dsc, j6_bkgrnd_rspctd_rtng, 
			j6_bkgrnd_rspctd_rtng_dsc, vlntr_hrs_value, vlntr_hrs_dsc, a9_dntd_bld_ind, b9_dntd_mny_ind, c9_prprdns_app_ind, 
			d9_hs_crs_ind, e9_ref_fmly_ind, f9_none_ind, l12mo_have_you_no_answer, a5_exp_rtng_num, a5_exp_rtng_dsc, 
			b5_exp_rtng_num, b5_exp_rtng_dsc, c5_exp_rtng_num, c5_exp_rtng_dsc, d5_exp_rtng_num, d5_exp_rtng_dsc, 
			e5_exp_rtng_num, e5_exp_rtng_dsc, f5_exp_rtng_num, f5_exp_rtng_dsc, g5_exp_rtng_num, g5_exp_rtng_dsc, 
			h5_exp_rtng_num, h5_exp_rtng_dsc, h5_in_rgn_dro_dplymnt_ind, h5_out_rgn_dro_dplymnt_ind, h5_both_rgn_dro_dplymnt_ind, 
			h5_no_rgn_dro_dplymnt_ind, i5_exp_rtng_num, i5_exp_rtng_dsc, j5_exp_rtng_num, j5_exp_rtng_dsc, k5_exp_rtng_num, 
			k5_exp_rtng_dsc, l5_exp_rtng_num, l5_exp_rtng_dsc, m5_exp_rtng_num, m5_exp_rtng_dsc, a10_vlntr_exprnc_easy_rtng, 
			a10_vlntr_exprnc_easy_rtng_dsc, b10_vlntr_cnctn_usfl_rtng, b10_vlntr_cnctn_usfl_rtng_dsc, c10_vlntr_cnctn_easy_rtng, 
			c10_vlntr_cnctn_easy_rtng_dsc, a11_arc_cmptr_rtng, a11_arc_cmptr_rtng_dsc, b11_arc_email_rtng, b11_arc_email_rtng_dsc, 
			c11_arc_xchng_rtng, c11_arc_xchng_rtng_dsc, d11_arc_edge_rtng, d11_arc_edge_rtng_dsc, arc_edctn_lvl_value, 
			arc_edctn_lvl_dsc, vlntr_dscrptn_value, vlntr_dscrptn_dsc, dw_create_ts, dw_updt_ts, row_stat_cd, appl_src_cd, 
			load_id, dm_cnst_zip_5_cd, attribtn_zip_5_cd, unit_key, mktg_unit_key, mktg_unit_cd, unit_nm, region_cd, region_nm, 
			division_cd, division_dsc, district_cd, district_nm, unit_zip, orc_division_id, orc_division_name, ds_region_id, 
			ds_region_name, geo_lat_lon_ID, geo_centriod_lat, geo_centriod_lon, bio_region_nm, bio_division_dsc, ldrshp_nm, fiscal_yr
		FROM ranked_data
		WHERE rn = 1;
		...
		WITH ranked_data AS (
			...
			FROM (
				SELECT 
					...
					CAST(NULL AS SMALLINT) m5_mrktg_ind, /* FY22 addition */
					vol_area_other AS l2_other_srvc_ind, 
					vol_area_no_answer,
					/* Volunteer Area: Experience Rating Follow Up for Indicated Volunteer Area FY22 addition */
					CAST(NULL AS SMALLINT) AS a5_exp_rtng_num,
					CAST(NULL AS VARCHAR(25)) AS a5_exp_rtng_dsc,
					CAST(NULL AS SMALLINT) AS b5_exp_rtng_num,
					CAST(NULL AS VARCHAR(25)) AS b5_exp_rtng_dsc,
					...
					CAST(NULL AS SMALLINT) AS h5_exp_rtng_num,
					CAST(NULL AS VARCHAR(25)) AS h5_exp_rtng_dsc,
					0 AS h5_in_rgn_dro_dplymnt_ind,     /* new FY24 column */
					0 AS h5_out_rgn_dro_dplymnt_ind,    /* new FY24 column */
					0 AS h5_both_rgn_dro_dplymnt_ind,   /* new FY24 column */
					0 AS h5_no_rgn_dro_dplymnt_ind,     /* new FY24 column */
					CAST(NULL AS SMALLINT) AS i5_exp_rtng_num,
					CAST(NULL AS VARCHAR(25)) AS i5_exp_rtng_dsc,
					CAST(NULL AS SMALLINT) AS j5_exp_rtng_num,
					CAST(NULL AS VARCHAR(25)) AS j5_exp_rtng_dsc,
					...
					END AS SMALLINT) AS rcmnd_frnd_vlntr_rtng,
					CAST(CASE
					...
					END AS VARCHAR(50)) AS rcmnd_frnd_vlntr_rtng_dsc,
					CASE
					...
					CAST(NULL AS SMALLINT) AS tmwrk_good_rtng,			/* new FY24 column */
					CAST(NULL AS VARCHAR(25)) AS tmwrk_good_rtng_dsc,	/* new FY24 column */
					CASE
					...
					CAST(CASE 
						...
						ELSE NULL
					END AS SMALLINT) AS vlntr_hrs_value,
					CAST(CASE 
		...
		WITH ranked_data AS (
			...
			FROM (
				SELECT	
					...
					vlntr_length,
					a5_acctg_fnc_ind, 
					b5_blood_svc_ind AS a2_blood_svc_ind, /* a2_blood_svc_ind, */
					c5_gen_admin_sprt_srvc_ind AS b2_gen_admin_sprt_srvc_ind,
					d5_saf_srvc_ind AS c2_saf_srvc_ind,  /* c2_saf_srvc_ind, */
					e5_comnty_srvc_ind AS d2_comnty_srvc_ind,
					f5_trng_srvc_ind AS e2_health_safety_srvc_ind,  /* e2_health_safety_srvc_ind, */
					g5_vlntr_srvc_ind AS f2_vlntr_srvc_ind,
					h5_dstr_srvc_ind AS g2_dstr_srvc_ind,
					i5_intl_srvc_ind AS h2_intl_srvc_ind,
					j5_youth_prgrm_srvc_ind AS i2_youth_prgrm_srvc_ind,
					k5_fr_srvc_ind AS j2_fr_srvc_ind,
					l5_ldrshp_ind AS k2_ldrshp_ind,
					CAST(NULL AS SMALLINT) AS m5_mrktg_ind, /* FY22 addition */
					CAST(NULL AS SMALLINT) AS l2_other_srvc_ind,
					CAST(NULL AS VARCHAR(3)) AS vol_area_no_answer,
					/* Volunteer Area: Experience Rating Follow Up for Indicated Volunteer Area FY22 addition */
					CAST(NULL AS SMALLINT) AS a5_exp_rtng_num,
					CAST(NULL AS VARCHAR(25)) AS a5_exp_rtng_dsc,
		...
		--4th insert
		/* FY 22 v5.0 (vmsAnnSurvey5) AND FY 23 v6.0 (vmsAnnSurvey6) AND FY24 v7.0 (vmsAnnSurvey7) */
		INSERT INTO mktg_stage_tbls.srvy_anvrsy_vlntr_rspns_all_stg
		WITH ranked_data AS (
			SELECT
				snapshot_ts, 
			...
			FROM (
				...
				SELECT
					...
					k5_fr_srvc_ind AS j2_fr_srvc_ind,
					l5_ldrshp_ind AS k2_ldrshp_ind,
					m5_mrktg_ind,                 /* FY22 addition */
					n5_other_ind AS l2_other_srvc_ind,     /* FY22 update */
					CAST(NULL AS VARCHAR(3)) AS vol_area_no_answer,
					/* Volunteer Area: Experience Rating Follow Up for Indicated Volunteer Area FY22 addition */
					a5_exp_rtng_num,
					a5_exp_rtng_dsc,
					...
					h5_exp_rtng_num,
					h5_exp_rtng_dsc,
					h5_in_rgn_dro_dplymnt_ind,    /* new FY24 column */
					h5_out_rgn_dro_dplymnt_ind,   /* new FY24 column */
					h5_both_rgn_dro_dplymnt_ind,  /* new FY24 column */
					h5_no_rgn_dro_dplymnt_ind,    /* new FY24 column */
					i5_exp_rtng_num,
					i5_exp_rtng_dsc,
					j5_exp_rtng_num,
					j5_exp_rtng_dsc,
					...
					CAST(c6_trng_mtrl_rtng AS SMALLINT) AS c6_trng_mtrl_rtng,
					CAST(c6_trng_mtrl_rtng_dsc AS VARCHAR(25)) AS c6_trng_mtrl_rtng_dsc,
					CAST(tmwrk_good_rtng AS SMALLINT) AS tmwrk_good_rtng,           /* new FY24 column */
					CAST(tmwrk_good_rtng_dsc AS VARCHAR(25)) AS tmwrk_good_rtng_dsc,    /* new FY24 column */
					CAST(d6_tmwrk_good_rtng AS SMALLINT) AS d6_tmwrk_good_rtng,
					CAST(d6_tmwrk_good_rtng_dsc AS VARCHAR(25)) AS d6_tmwrk_good_rtng_dsc,
					CAST(e6_team_value_rtng AS SMALLINT) AS e6_team_value_rtng,
					CAST(e6_team_value_rtng_dsc AS VARCHAR(25)) AS e6_team_value_rtng_dsc,
					...
					f9_none_ind,
					CAST(NULL AS SMALLINT) AS l12mo_have_you_no_answer, /* This was on the previous survey but not on current version */
					/* FY22 survey re-added the below questions - removed in FY21 */
					a10_vlntr_exprnc_easy_rtng,
					a10_vlntr_exprnc_easy_rtng_dsc,
					b10_vlntr_cnctn_usfl_rtng,
					b10_vlntr_cnctn_usfl_rtng_dsc,
					c10_vlntr_cnctn_easy_rtng,
					c10_vlntr_cnctn_easy_rtng_dsc,
					/* Were not re-added - removed in FY21 and FY22 */
					CAST(NULL AS SMALLINT) AS a11_arc_cmptr_rtng,
					CAST(NULL AS VARCHAR(25)) AS a11_arc_cmptr_rtng_dsc,
					CAST(NULL AS SMALLINT) AS b11_arc_email_rtng,
					CAST(NULL AS VARCHAR(25)) AS b11_arc_email_rtng_dsc,
		...
		Truncate table mktg_ops_tbls.srvy_anvrsy_vlntr_rspns_all;
		
		-- Insert data from staging to target
        INSERT INTO mktg_ops_tbls.srvy_anvrsy_vlntr_rspns_all
        SELECT * FROM mktg_stage_tbls.srvy_anvrsy_vlntr_rspns_all_stg;
		
		v_end_time := GETDATE();
		v_ok_message = 'Records inserted.';
        
        UPDATE etl_config.audit_log
		SET status = 'Complete', end_time = v_end_time, TaskMessage = v_ok_message, recs_processed = cast((select count(*) from mktg_stage_tbls.srvy_anvrsy_vlntr_rspns_all_stg) as INTEGER)
        WHERE proc_name = 'ld_srvy_anvrsy_vlntr_rspns_all' 
    ...
    EXCEPTION
		WHEN OTHERS THEN
			v_end_time := GETDATE(); 
			v_error_message := 'Error in ld_srvy_anvrsy_vlntr_rspns_all: ' || SQLERRM;
        
			-- Log the error before raising the exception
			INSERT INTO etl_config.audit_log (proc_name, task_name, status, start_time, end_time, TaskMessage)
			VALUES ('ld_srvy_anvrsy_vlntr_rspns_all', 'Stored Procedure', 'ERROR', v_start_time, v_end_time, v_error_message);
