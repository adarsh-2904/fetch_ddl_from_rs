CREATE OR REPLACE VIEW mktg_ops_vws.srvy_vlntr_anvrsry_los
/*
...
from the srvy_vlntr_los view in that it reflects the volunteer LOS at the time the Volunteer anniversary survey was sent to the volunteer.  We created
...
(
    ...
    WHERE 
        b.delivery_label LIKE '%VMS Anniversary Satisfaction Survey%Initial%'
...
QUALIFY Row_Number() Over (PARTITION BY cnst_mstr_id, delivery_label  ORDER BY  srvy_sent_dt ASC) = 1
